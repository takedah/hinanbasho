function getLocation() {
    ...
    if(navigator.geolocation) {
        ...
        function geoError(error) {
            ...
            switch(errorCode) {
                case TIMEOUT:
                    setGpsServiceStatus("タイムアウトしました。しばらく待ってから再度お試しください。");
                    break;
                case PERMISSION_DENIED:
                    setGpsServiceStatus("位置情報の取得を許可して再度お試しください。");
                    break;
                case POSITION_UNAVAILAVLE:
                    setGpsServiceStatus("位置情報の取得中にエラーが発生しました。再度お試しください。");
                    break;
            };
        };
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("useGps").addEventListener("click", getLocation, false);
}, false);
